...
Phase 10 (Hardening & Security Review) is done for its own scoped done-criterion, established via a dedicated audit run before any code was written rather than assumed: all 22 tables across every prior migration already had RLS enabled with at least one policy — the Phase 9 `platform_settings` gap was an isolated miss, not a pattern — so this phase targeted the three real, concrete gaps `CLAUDE.md` itself names by section: MFA on fund-release actions (Section 3.7 / Section 7 risk register), an audit log (Section 6), and rate limiting beyond chat (Section 3.6). Built: `has_verified_mfa_factor()`/`session_is_aal2()`/`require_fund_release_mfa()` (migration `20260807130000`), gating `approve_payout`, `complete_payout`, `confirm_contribution` (only on the `uzuza_held` custody path — group-owned confirmations don't move funds through Uzuza, so they stay ungated), and `set_account_type` (only when switching *into* `uzuza_held`) behind a verified TOTP factor plus an `aal2` session, with opt-in enrollment at `/profile/security` (`components/MfaEnrollment.tsx`, using Supabase's built-in `auth.mfa` API — no custom TOTP backend needed); a platform-wide `audit_log` table + `log_audit_event()`/`list_audit_log()` (migration `20260807140000`), threaded through every high-stakes RPC (payout approval/completion/request, contribution confirmation into custody, account-type changes, group-change proposals/approvals, member removal, exit/pause decisions, missed-payment reports, and every staff decide-action from Phase 9), viewable at `app/internal/audit/`; and generalized rate limiting (migration `20260807150000`) — the exact "reject if a matching row exists within N seconds" pattern already proven for chat, now covering `submit_contribution_proof`/`request_mediation`/`log_unmatched_payment` (5s) and `request_payout` (60s, a deliberate-action guard more than an abuse one), backed by a new shared `rate_limit_events` table. OTP request throttling itself needed no new code — Supabase's own auth config (`rate_limit_otp`, `sms_max_frequency`) already covers it, confirmed during the audit rather than assumed. `next.config.ts` gained baseline security headers (CSP scoped to Supabase's origin plus the QR endpoint, `X-Frame-Options: DENY`, `nosniff`, a strict referrer policy); `npm audit` reported zero vulnerabilities, nothing to fix.

This phase's own live e2e check (`scripts/e2e-phase10-check.mjs`) caught **a real bug** the same day it was written: `rate_limit_events.user_id` and `staff_users.user_id` (the latter dating back to Phase 9, migration `20260807120000`) both referenced `auth.users(id)` with no `ON DELETE` behavior specified, defaulting to `NO ACTION` — this made `auth.admin.deleteUser()` fail outright ("Database error deleting user") for any user who had ever triggered a rate-limited action or been granted staff access, since Postgres refused to leave a dangling FK. The `staff_users` half of this had been live and unnoticed since Phase 9 because no earlier e2e script happened to grant staff access to a test user *and* delete that same user in one run. Fixed in migration `20260807160000`: both now cascade-delete (neither table is meant to outlive the user), while `audit_log.actor_user_id` deliberately got the opposite fix — `ON DELETE SET NULL`, since an audit trail should survive the actor being deleted later, not disappear with them.

One thing this phase could **not** fully verify live, and says so rather than papering over it: real TOTP enrollment (`auth.mfa.enroll()`) fails against this project's hosted Supabase instance with a server-side `500 "Error generating QR Code"` — reproduced identically via raw REST calls and the official `supabase-js` client, with zero rows ever reaching `auth.mfa_factors`, so it's a GoTrue-side infrastructure issue, not anything in Uzuza's code or schema. `scripts/e2e-phase10-check.mjs` still verifies the property that actually matters — an admin with no verified MFA factor is blocked from every fund-release RPC with a clear, actionable error message, not a silent failure — and gracefully skips only the post-enrollment assertions (a successful approval/completion with a verified `aal2` session) when this upstream error occurs, logging a clear warning rather than a false pass. Worth a Supabase support ticket before relying on this in production; until then, MFA *enrollment* is unverified end-to-end even though MFA *enforcement* is fully proven.
...
- `scripts/e2e-check.mjs` through `scripts/e2e-phase10-check.mjs` are repeatable regression checks against the real deployed backend — they require `sms_test_otp` to be temporarily set in Supabase's auth config (Authentication → Sign In / Providers → Phone → Test Phone Numbers and OTPs; also settable via the Management API — the exact format is a comma-separated `<phone-without-plus>=<code>` list, e.g. `250788000111=123456`), and clean up their own test data on success.
- Any table with a column referencing `auth.users(id)` needs its `ON DELETE` behavior chosen deliberately (`cascade` for ephemeral/ownership data like `rate_limit_events`/`staff_users`, `set null` for records that should outlive the user like `audit_log`) — the default `NO ACTION` silently breaks `auth.admin.deleteUser()` the first time a real row exists, caught twice now (Phase 9's `staff_users`, Phase 10's own `rate_limit_events`) only because an e2e script happened to both create the referencing row and delete the user in the same run.
